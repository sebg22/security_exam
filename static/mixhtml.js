...
async function mix_fetch_data(el) {
  ...
  if (method == "post" || method == "put") {
    ...
    for (let i = 0; i < attrs.length; i++) {
      ...
      // #########This part is made with chatGPT. It validates that the user has uploaded 3 images in create item#####
      // Handle file input separately
      if (attrs[i].type === "file") {
        const expectedCount = parseInt(attrs[i].getAttribute("mix-check"), 10); // Get required number
        const actualCount = attrs[i].files.length; // Get number of uploaded files
        if (actualCount !== expectedCount) {
          cl(`File input validation failed: Expected ${expectedCount}, got ${actualCount}`);
          attrs[i].classList.add("mix-error");
          errors = true;
        }
        continue; // Skip regex validation for file inputs
      }
      // ############################################################################################
